...
uint8_t buffer[100];
uint32_t GPS_Time;
long latitude,longitude,GPS_Height,GPS_Height_MSL,GroundSpeed ;
...
uint8_t enableNAV_PVT[] ={0xB5 ,0x62 ,0x06 ,0x01 ,0x08 
...
void initGPS()
{
 ...
 delay(10);
 sendUBXcommand(disableGPRMC,sizeof(disableGPRMC));
 delay(10);
 sendUBXcommand(enableNAV_PVT,sizeof(enableNAV_PVT));
 delay(10);
...
// LoRa initialization
 ...
 if (!initBMP()) {
  ...
  delay(500);
  Serial.println("Ready to Rock'n'Roll !");
...
void loop()
{
    ...
    if(curtime - prevLoRatime >=2000)
 {
    ...
    LoRastring = String(curtime)+","+String(GPS_Time)+","+String(latitude)+","+String(longitude);
     char msg[100] ;
   ...
   if(Serial1.available()>50)
   {
    ...
    if(Serial1.peek() == 0xB5)
    {
     ...
     if((Serial1.read()==0x62) && (Serial1.read()==0x01) && (Serial1.read()==0x07))
     {
      ...
      GPS_Time =(uint32_t)((uint32_t)buffer[5]<<24 | (uint32_t)buffer[4]<<16 | (uint32_t)buffer[3]<<8 | (uint32_t)buffer[2]);
       
     //for(int i =0;i<100;i++)
     //Serial.println(buffer[i],HEX); 
     //Serial.println(GPS_Time,HEX); 
     //while(1);
     }
    }
    else
 ...
 */
   ...
   data = String(millis()) + "," +       + "," + String(ax) + "," + String(ay) + "," + String(az) + "," + String(gx) + "," + String(gy) + "," + String(gz) + "," + String(pressure) + "," + String(temperature) ;
  // data = String(ax);
...
void readmpudata()
{
 ...
 SPI.transfer(accdataadd | 0x80);
 
  uint16_t data = (uint8_t)SPI.transfer(0x00)<<8;
  data |= (uint8_t)SPI.transfer(0x00);
  ax = (int16_t)data;

  data = (uint8_t)SPI.transfer(0x00)<<8;
  data |= (uint8_t)SPI.transfer(0x00);
  ay = (int16_t)data;
